...
export const listProductFiles = createServerFn({ method: "GET" })
  .middleware([authMiddleware])
  .validator((productId: number) => Number(productId))
...
export const registerProductFile = createServerFn({ method: "POST" })
  ...
  .validator((input: {
    ...
    kind: input.kind === "cover" ? "cover" : "delivery",
  }))
...
export const removeProductFile = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context, data: fileId }) => {
    ...
    const rows = await sql.query<{ object_key: string }>(
      "select object_key from product_files where id = $1 and user_id = $2 limit 1",
